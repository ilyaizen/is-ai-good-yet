  ...
  return {
    dimensions: Object.fromEntries(
      V2_DIMENSIONS.map((name) => {
        ...
        return [name, {
          applicability: value.applicability === "explicit" || value.applicability === "implicit" ? value.applicability : "not_addressed",
...
function mapCommunity(raw: Record<string, unknown> | null | undefined) {
  ...
  const mapped = Object.fromEntries(V2_DIMENSIONS.map((name) => {
    const value = dimensions[name] ?? {};
    const dissent = value.dissent as Record<string, unknown> | null | undefined;
    return [name, {
      ...mapSource({ dimensions: { [name]: value } }).dimensions[name],
      visibilityWeightedScore: typeof value.visibility_weighted_score === "number" ? value.visibility_weighted_score : null,
      diversityBalancedScore: typeof value.diversity_balanced_score === "number" ? value.diversity_balanced_score : null,
...
function mapStory(raw: RawStory): V2StoryCard {
  ...
  const scopes = ((raw.article?.result as Record<string, unknown> | undefined)?.scopes ?? ["general"]) as V2StoryCard["scopes"];
  return {
    ...
    community,
    combined: {
      dimensions: Object.fromEntries(V2_DIMENSIONS.map((name) => [name, {
      ...
      addressedDimensions: Array.isArray(combinedRaw.addressedDimensions) ? combinedRaw.addressedDimensions as V2Dimension[] : []
    },
    sourceDivergence: Object.fromEntries(V2_DIMENSIONS.map((name) => [name, typeof sourceDivergence[name] === "number" ? sourceDivergence[name] : null])) as V2StoryCard["sourceDivergence"]
  };
}
