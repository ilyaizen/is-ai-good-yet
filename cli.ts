...
async function configureBackfillArgs(): Promise<string[] | null> {
  ...
  const output = await text({
    message: "Output file:",
    defaultValue: "data/histre_feed.json",
    placeholder: "data/histre_feed.json",
  })
  if (isCancel(output)) throw new Error("Operation cancelled")
  if (output && String(output).trim()) args.push("--output", String(output))

  const recent = await text({
    message: "Recent output file (optional):",
    defaultValue: "",
    placeholder: "data/recent_links.json",
  })
...
async function configureScraperArgs(): Promise<string[] | null> {
  ...
  if (mode === "scrape-new")
    return ["--lean", "--stealth-mode=seleniumbase", "--no-headful-switch", "-b", "80", "-c", "6", "-r", "2.5"]
  if (mode === "retry-light")
    return [
      "--lean",
      "--retry-failed",
  ...
  if (mode === "fast")
    return ["--lean", "--stealth-mode=seleniumbase", "--no-headful-switch", "-b", "100", "-c", "10", "-r", "5"]
  ...
  while (true) {
    const action = await select({
      ...
      options: [
        ...
        {
          value: "pipeline-v2-prefilter",
          label: "V2: Broad Prefilter",
          hint: "Classify all AI domains without changing V1",
        },
        {
          value: "pipeline-v2-comments",
          label: "V2: Collect HN Comments",
          hint: "Fetch and select representative discussion",
        },
        ...
        {
          value: "pipeline-v2-run",
          label: "V2: Run Isolated Pipeline",
          hint: "Prefilter → comments → analysis → atomic export",
        },
        ...
        {
          value: "python-setup",
          label: `Setup Python Env${venvReady ? " ✓" : " ⚠ venv missing"}`,
          hint: "Create venv + install deps",
        },
    ...
    try {
      if (action === "dev") {
        await runScript("vp", ["dev"])
      } else if (action === "build") {
        await runScript("vp", ["build"])
      } else if (action === "start") {
        await runScript("vp", ["run", "start"])
        ...
        await runScript(
          pythonCmd,
          ["src/reclassify_sterile.py", ...(dryRun ? ["--dry-run", "-v"] : ["-v"])],
          PIPELINE_DIR
        )
      } else if (action === "pipeline-bootstrap") {
        await runScript("vp", ["run", "pipeline:bootstrap"])
      } else if (action === "pipeline-stats") {
        await runScript("vp", ["run", "pipeline:stats"])
        ...
        if (!fs.existsSync(venvDir)) {
          ...
          if (!ok) {
            console.error("venv creation failed.")
            continue
          }
        } else {
