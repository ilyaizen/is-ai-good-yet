<script lang="ts">
  ...
  let { article, community, articleConfidence, communityConfidence }: Props = $props();
...
<div class="v2-axis" data-conflict={conflict}>
  ...
  <div class="v2-axis__values"><span>ARTICLE {article === null ? "N/A" : `${article >= 0 ? "+" : ""}${article.toFixed(2)}`}</span><span>COMMUNITY {community === null ? "N/A" : `${community >= 0 ? "+" : ""}${community.toFixed(2)}`}</span></div>
</div>
